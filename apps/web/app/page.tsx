...
export default function Home() {
  return <main>
    ...
    <section id="engineering" className="section section--engineering" aria-labelledby="engineering-title"><div className="section-shell"><SectionHeading eyebrow="02 / ENGINEERING" title="THINK. DESIGN. BUILD." /><div className="engineering-lab" id="engineering-title">{skills.map((skill, i) => <span key={skill} style={{ "--i": i } as React.CSSProperties}>{skill}</span>)}<div className="lab-core">SYSTEM<br /><b>→</b><br />PROJECT</div></div><p className="section-note">Technology is most useful when it is connected to a real problem, a real system, or a real piece of work.</p></div></section>

    <section id="projects" className="section section--projects" aria-labelledby="projects-title"><div className="section-shell"><SectionHeading eyebrow="03 / PROJECTS" title="WHERE IDEAS TAKE FORM." accent="violet" /><p className="statement" id="projects-title">WHAT I BUILD.</p><div className="project-grid">{projects.slice(0, 3).map((project, index) => <TransitionLink data-cursor="VIEW" className="project-card" href={`/projects/${project.slug}`} key={project.slug}><span className="project-index">0{index + 1}</span><span className="project-category">{project.category.join(" / ")}</span><div className="project-art" aria-hidden="true"><span /></div><h3>{project.title}</h3><p>{project.problem}</p><span className="project-arrow">VIEW ↗</span></TransitionLink>)}</div><TransitionLink className="text-link" href="/projects">EXPLORE ALL PROJECTS ↗</TransitionLink></div></section>
...
>
  <div className="section-shell">
    ...
    <section id="contact" className="section section--contact" aria-labelledby="contact-title"><div className="section-shell contact-block"><span className="eyebrow">08 / CONTACT</span><h2 id="contact-title">EVERYTHING STARTS WITH A FIRST STEP.</h2><p>Have an idea, opportunity, or problem worth discussing?</p><div className="contact-actions"><MagneticButton href={`mailto:${site.email}`}>EMAIL ME</MagneticButton><MagneticButton copy>COPY EMAIL</MagneticButton></div><div className="socials"><a data-cursor="LINK" href={site.socials.linkedin} target="_blank" rel="noreferrer">LinkedIn</a><a data-cursor="LINK" href={site.socials.github} target="_blank" rel="noreferrer">GitHub</a><a data-cursor="LINK" href={site.socials.instagram} target="_blank" rel="noreferrer">Instagram</a></div></div></section>
